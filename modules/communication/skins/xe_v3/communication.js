...
/* 친구 그룹 관리 토글 */
jQuery(function($){
	var ctrTo = $('.ctrArea>.ctrGroup>.ctrTo');
	ctrTo.hide().addClass('off');
	$('.ctrArea>.ctrGroup .ctrToBtn').click(function(){
		if(ctrTo.hasClass('off')){
			ctrTo.fadeIn().removeClass('off');
		} else {
			ctrTo.fadeOut().addClass('off');
		};
		var arrow = $(this).children('span');
		if(arrow.hasClass('arrLeft')){
			arrow.removeClass().addClass('arrRight');
		} else {
			arrow.removeClass().addClass('arrLeft');
		};
	});
});