 ...
 **/
class ncenterController extends ncenter
{
	function triggerAfterDeleteMember($obj)
	{
		...
		executeQuery('ncenter.deleteNotifyByMemberSrl', $args);
	...
	function triggerBeforeModuleObjectProc(&$oModule)
	{
		...
		{
			$flag_path = './files/ncenter/new_message_flags/';

			$need_update = false;
			// [XE 1.5.x] 쪽지 알림 메시지 체크
			if(strpos(Context::getHtmlFooter(), 'xeNotifyMessage') !== FALSE)
			{
				$need_update = true;
			}
			// [XE 1.4.x] 쪽지 알림 메시지 체크
			else if(strpos(Context::getHtmlFooter(), 'act=dispCommunicationNewMessage') !== FALSE)
			{
				$need_update = true;
			}
			// 메시지 플래그 파일 체크
			...
			else if($need_update)
			{
				...
				$output = executeQuery('ncenter.getCountNewMessage', $args);
				...
				$output = executeQuery('ncenter.updateNotifyReadedByType', $cond);
	...
	function triggerAfterDeleteComment(&$obj)
	{
		...
		$args = new stdClass;
		$args->srl = $obj->comment_srl;
		$output = executeQuery('ncenter.deleteNotifyBySrl', $args);
		return new Object();
	}
	...
	function triggerAfterModuleHandlerProc(&$oModule)
	{
		...
		if($oModule->getLayoutFile() == 'popup_layout.html') Context::set('ncenter_is_popup', TRUE);

		$oNcenterModel = &getModel('ncenter');
		$config = $oNcenterModel->getConfig();
		if($config->use != 'Y') return new Object();

		$this->_hide_ncenter = false;
		if($oModule->module == 'beluxe' && Context::get('is_modal'))
		{
			$this->_hide_ncenter = true;
		}
		...
		if($oModule->act == 'dispBoardReplyComment')
		{
			...
			if($comment_srl && $logged_info)
			{
				$args->target_srl = $comment_srl;
				$args->member_srl = $logged_info->member_srl;
				executeQuery('ncenter.updateNotifyReadedByTargetSrl', $args);
			}
		}
				...
				if($_comment_list)
				{
					...
					if($need_check_socialxe)
					{
						...
						if($oDB->isTableExists('socialxe'))
						{
							...
							$output = executeQuery('ncenter.getSocialxeCount', $args);
							if($output->data->cnt)
							{
		...
		else if(($oModule->act == 'dispKinWrite' || $oModule->act == 'procKinInsert') && $config->block_kin_modify == 'Y')
		{
			// 답변이 있을 때 질문 수정 제한
			$oDocumentModel = &getModel('document');
			$oDocument = $oDocumentModel->getDocument($vars->document_srl);
			...
			{
				$grant = $oModuleModel->getGrant($oModule->module_info, $logged_info);
				if($oDocument->getCommentCount() > 0 && !$grant->is_admin && !$grant->manager) return new Object(-1, '답변이 작성된 질문은 수정할 수 없습니다.');
			}
		}
	...
	{
		// 팝업창이면 중지
		if(Context::get('ncenter_is_popup')) return;
		...
		Context::set('ncenter_config', $config);

		$oModuleModel = &getModel('module');
		$ncenter_module_info = $oModuleModel->getModuleInfoXml('ncenter');
		$jsCacheRefresh = '?'.$ncenter_module_info->version.'.'.$ncenter_module_info->date.'.js';
		Context::addJsFile('./modules/ncenter/tpl/js/ncenter.js'.$jsCacheRefresh, true, '', 100000);


		$oNcenterModel = &getModel('ncenter');

		// 알림 목록 가져오기
		$_output = $oNcenterModel->getMyNotifyList();
		...
		Context::set('ncenter_latest_notify_id', $_latest_notify_id);

		if($_COOKIE['_ncenter_hide_id'] && $_COOKIE['_ncenter_hide_id'] == $_latest_notify_id) return;
		setcookie('_ncenter_hide_id', '', 0, '/');
		...
		Context::set('ncenter_list', $_output->data);
		Context::set('ncenter_page_navigation', $_output->page_navigation);
	...
	{
		$oModuleModel = &getModel('module');
		$module_info = $oModuleModel->getModuleInfoXml('ncenter');
		$cacheRefresh = '?'.$module_info->version.'.'.$module_info->date.'.css';
		if(file_exists(FileHandler::getRealPath($this->template_path . 'ncenter.css')))
		{
			Context::addCssFile($this->template_path . 'ncenter.css'.$cacheRefresh, true, 'all', '', 100);
		}

		$oNcenterModel = &getModel('ncenter');
		$config = $oNcenterModel->getConfig();
		if($config->colorset && file_exists(FileHandler::getRealPath($this->template_path . 'ncenter.' . $config->colorset . '.css')))
		{
			Context::addCssFile($this->template_path . 'ncenter.'.$config->colorset.'.css'.$cacheRefresh, true, 'all', '', 100);
		}

		if($config->zindex)
		{
			Context::set('ncenter_zindex', ' style="z-index:' . $config->zindex . ';" ');
		}
		...
		if(Mobile::isFromMobilePhone())
		{
			...
			Context::addCssFile($this->template_path . 'ncenter.mobile.css'.$cacheRefresh, true, 'all', '', 100);
		}
	}
	...
	function _getTemplate()
	{
		...
		$result = $oTemplateHandler->compile($path, 'ncenter.html');
	...
	function updateNotifyRead($notify, $member_srl)
	{
		...
		$args->member_srl = $member_srl;
		$args->notify = $notify;
		//$output = executeQuery('ncenter.updateNotifyReaded', $args);
		$output = executeQuery('ncenter.deleteNotify', $args);
	...
	function updateNotifyReadiByTargetSrl($target_srl, $member_srl)
	{
		...
		//$output = executeQuery('ncenter.updateNotifyReadedByTargetSrl', $args);
		$output = executeQuery('ncenter.deleteNotifyByTargetSrl', $args);
	...
	function updateNotifyReadAll($member_srl)
	{
		$args = new stdClass;
		$args->member_srl = $member_srl;
		//$output = executeQuery('ncenter.updateNotifyReadedAll', $args);
		$output = executeQuery('ncenter.deleteNotifyByMemberSrl', $args);

		return $ouptut;
	}

	function procNcenterNotifyRead()
	{
		$logged_info = Context::get('logged_info');
	...
	function procNcenterNotifyReadAll()
	{
		$logged_info = Context::get('logged_info');
	...
	function _isAnonymous($source_type, $triggerObj)
	{
		...
		// EX 익명 체크박스
		if($module_info->module == 'bodex' && $triggerObj->is_anonymous == 'Y') return TRUE;
		// DX 익명 체크박스
		...
		{
			// EX 댓글 익명 강제
			if($module_info->module == 'bodex' && $module_info->use_anonymous_comment == 'R') return TRUE;
			// DX 익명 강제
		...
		{
			// EX 문서 익명 강제
			if($module_info->module == 'bodex' && $module_info->use_anonymous == 'R') return TRUE;
			// DX 익명 강제
			if($module_info->module == 'beluxe' && $module_info->use_anonymous == 'Y') return TRUE;
		}
	...
	function _insertNotify($args, $anonymous = FALSE)
	{
		...
		$output = executeQuery('ncenter.insertNotify', $args);
		return $output;
	}
	...
	function _getMentionTarget($content)
	{
		...
		foreach($nicks as $nick_name)
		{
			...
			$output = executeQuery('ncenter.getMemberSrlByNickName', $vars);
