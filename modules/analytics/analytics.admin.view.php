<?php
	class analyticsAdminView extends analytics{
		...
		function _setDefaultData($method, $end_date=null){
			$oModuleModel = &getModel('module');
			...
			if(!$end_date) $end_date = $this->_getEndDate(Context::get('end_date'));
		...
		function dispAnalyticsVisitInfoAdminVisitChart(){
			...
			if (!$api_key)
			{
				header('Location:'.getNotEncodedUrl('', 'module', 'admin', 'act', 'dispAnalyticsAdminContent'));
				return;
			}
			...
			$this->setTemplateFile('visit_info');
		}
		...
		function dispAnalyticsVisitInfoAdminVisitBackChart(){
			...
			$day = 24*60*60;
			$initial_search_date = 7;
			$end_date = Context::get('end_date');

			// 초기 검색기준을 7일 전으로 설정
			if($end_date == '' || !$end_date) $end_date = date('Y-m-d', time()-($initial_search_date * $day));
			else $end_date = date('Y-m-d', strtotime($end_date));
			
			$param = $this->_setDefaultData('visitBack', $end_date);
		...
		function _dispPageContent($method){
			...
			$avg_url = $this->_getJSONData($param['api_key'], 'pageAVGTime', array('start_date' => $param['start_date'], 'end_date' => $param['end_date']));
			Context::set('avg_url', $avg_url);
