<?php
	...
	if($obj->action == 'contestUpload'){
		...
		$args->extra_vars->a_contact_post2 = $obj->a_contact_post2;
...
<script type="text/javascript">
 jQuery(function($){
	...
	$('#contest_up_form').submit(function(){
		...
		if(!agree_check){
			error = 1;
			alert('공모작 공개 동의서를 읽고 동의해주셔야 제출이 가능합니다.');
			return false;
		}
		...
		if(!document_srl){
			...
			thumbnail_name = thumbnail_name.toLowerCase();
			if(!thumbnail_name || (thumbnail_suffix != 'jpg' && thumbnail_suffix != 'jpeg' && thumbnail_suffix != 'png' && thumbnail_suffix != 'gif')){
				error = 1;
				alert('썸네일이 올바르지 않습니다.\n jpg, jpeg, png, gif, bmp 만 가능합니다.');
				return false;
			}

			var psd_name = $('#up_psd').attr('value');
			psd_name = psd_name.toLowerCase();
			var psd_suffix = psd_name.substring(psd_name.lastIndexOf(".")+1);
			if(!psd_name || (psd_suffix != 'psd' && psd_suffix != 'zip')){
				error = 1;
				alert('파일이 올바르지 않습니다. psd, zip 만 가능합니다.');
				return false;
			}
		}else{
			var thumbnail_name = $('#up_thumbnail').attr('value');
