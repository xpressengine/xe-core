<?php
    ...
    class DBMysql extends DB {
		...
		var $oMemcache;
		var $use_memcache = false;
		var $time_memcache = 30;
...
//			$this->password = md5($this->password);
			...
			if($this->use_memcache)
			{
				...
				$this->oMemcache->connect('10.24.4.100','11211');
			}
        }
        ...
        function close() {
			...
			if($this->use_memcache && $this->oMemcache)
			{
        ...
        function _query($query) {
            ...
            // 오류 체크
            if(mysql_error($this->fd)){
				$no = mysql_errno($this->fd);
				$error = mysql_error($this->fd);
				$this->setError($no, $error);
				debugPrint($query .':'.$no.",".$error, true, '_query_error_.php');
			}
...
<<<<<<< .mine
			...
			$is_cache = true;
			$no_cache_tables = array('documents','comments','resourceapi_reported');
			foreach($output->_tables as $v)
			{
				if(in_array($v, $no_cache_tables)){
					$is_cache = false;
					break;
				}
			}
...
//					debugPrint($query);
			...
			if(!$cached)
			{
				...
				if($this->use_memcache && $this->oMemcache)
				{
					$this->oMemcache->set($key, $data, MEMCACHE_COMPRESSED, $this->time_memcache+(rand(1,10)*10));
				}
			}
			else
			{
...
>>>>>>> .r8167
				...
				if(!$cached)
				{
					...
					if($this->use_memcache && $this->oMemcache)
					{
						$this->oMemcache->set($key, $total_count, MEMCACHE_COMPRESSED, 10);
					}
				}
				else
				{
