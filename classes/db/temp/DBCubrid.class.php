<?php
    ...
    class DBCubrid extends DB
    {
        ...
        function _query($query)
        {
            ...
            if(!$result) debugPrint('null result: '. $query, true, '_query_error_.php');
            ...
            if (cubrid_error_code ()) {
                ...
                debugPrint($code . ' ,'. $msg .': '.$query, true, '_query_error_.php');
        ...
        function _executeSelectAct ($output)
        {
			...
			if(!in_array($output->tables,array('comments','documents')))
			{
				if($this->use_memcache && $this->oMemcache)
				{
					$key = md5($query);
			...
			if(!$cached)
			{
				...
				if($this->use_memcache && $this->oMemcache)
				{
					$this->oMemcache->set($key, $data, MEMCACHE_COMPRESSED, $this->time_memcache);
				}
			}
			else
			{
...
<<<<<<< .mine
            ...
            if ($total_count === false) {
				...
				if(!$cached)
				{
					...
					if($this->use_memcache && $this->oMemcache)
					{
						$this->oMemcache->set($key, $count_output, MEMCACHE_COMPRESSED, $this->time_memcache);
					}
				}
				else
				{
